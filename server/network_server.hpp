...
// ==================== STRUCT CLIENTINFO ====================
/**
 * @brief Cấu trúc lưu trữ thông tin của mỗi client kết nối đến server.
 * 
 * Mỗi client khi kết nối sẽ có một đối tượng ClientInfo riêng để:
 * - Lưu trữ dữ liệu nhận được từ client (buffer)
 * - Bảo vệ dữ liệu khỏi xung đột khi nhiều luồng truy cập (mutex)
 * - Lưu tên đăng nhập của client (username)
 */
struct ClientInfo
{
    // Buffer lưu trữ dữ liệu nhận được từ client
    // Sử dụng vector<uint8_t> để lưu dữ liệu nhị phân (binary data)
    // Khi nhận dữ liệu từ socket, ta thêm vào buffer này
    // Khi đủ dữ liệu để tạo thành một packet hoàn chỉnh, ta lấy ra và xử lý
    std::vector<uint8_t> buffer;
    
    // Mutex (khóa) để bảo vệ buffer khỏi bị truy cập đồng thời từ nhiều luồng
    // Khi một luồng đang đọc/ghi buffer, các luồng khác phải đợi
    // Điều này tránh lỗi "race condition" (xung đột dữ liệu)
    std::mutex mutex;
    
    // Tên đăng nhập của client
    // Ban đầu là chuỗi rỗng "", sau khi client đăng nhập thành công sẽ được gán giá trị
    // Dùng để định danh client theo tên thay vì chỉ dùng file descriptor (số)
    std::string username = "";
};
...
private:
    ...
    // Đây là đặc điểm của Singleton pattern - chỉ có thể tạo instance thông qua getInstance()
    NetworkServer() : server_fd(-1)  // Khởi tạo server_fd = -1 (chưa có socket)
    {
...
public:
    ...
    NetworkServer &operator=(const NetworkServer &) = delete;  // Ngăn assignment operator
     ...
     *
     * Hàm này tìm kiếm client theo username, sau đó gọi sendPacket() với file descriptor.
     * Tiện lợi hơn khi ta chỉ biết username mà không biết file descriptor.
     * 
     ...
     * @return true nếu tìm thấy user và gửi thành công, false nếu không tìm thấy hoặc gửi thất bại
     */
     ...
     * - TCP là stream protocol (dòng dữ liệu liên tục), không có ranh giới gói tin
     ...
     * @return true nếu nhận được một packet hoàn chỉnh, false nếu chưa đủ dữ liệu hoặc kết nối đóng
     */
     ...
     * 
     * Sử dụng getpeername() để lấy thông tin địa chỉ của peer (client) đang kết nối.
     * Hữu ích cho việc ghi log, kiểm tra bảo mật, v.v.
     *
     * @param client_fd File descriptor của client.
     * @return Địa chỉ IP dưới dạng chuỗi (ví dụ: "192.168.1.100"), hoặc rỗng nếu thất bại.
     */
     ...
     * - delete &getInstance() là UNDEFINED BEHAVIOR vì getInstance() trả về static local variable
